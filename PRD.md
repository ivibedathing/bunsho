...
| 2026-07-15 | **Cream "paper" document surface retracted — the editor and published view now render on a dark surface** (`--paper: #211d18`, ink/lines/links re-tuned for dark) matching the rest of the carbon UI; the `paper` token and Card variant names stay | User feedback: the light editor canvas was too bright against the dark shell. Token-level change only, so both surfaces stay consistent and a light theme remains possible later |
| 2026-07-15 | **Profile page (`/profile`) with per-user activity read straight from the audit log** (no separate feed table; `recentActivityByUser` filters hash-chained entries by actor and batch-resolves document targets), reached via the sidebar user card. **Document detail gets a "Last change" right rail** (latest version event: author, time, note — viewers see published-only) linking to a new **`/documents/[id]/history` page**, where the change-log and diff/restore version tables moved from the detail page | User request. Audit log as the single activity source keeps the feed tamper-evident for free; splitting history out declutters the document view while keeping diffs one click away |

| 2026-07-15 | **Document attachments (files + inline editor images)** — new `attachments` table stores bytes in Postgres (`bytea`, 20 MB/file cap) with sha256 fingerprint; new write-once audit actions `attachment_added`/`attachment_deleted`; files served only via `/api/attachments/[id]` (auth + org-scoped on every read; viewers reach only attachments of current published docs; safe raster images render inline, everything else — incl. SVG — forces download with `nosniff` + `default-src 'none'` CSP). Editor gains an image-upload toolbar button (TipTap Image, `inline: true`) and the pinned serializer/parser round-trips `![alt](src)`; detail page gets an Attachments right-rail panel (upload/download/delete). Server-action body cap raised to 25 MB | Attachments in Postgres keep the deployment at exactly two services (§4.5/§8) and inside `pg_dump` backups; inline-as-GFM-image keeps frozen Markdown faithful (§6). Trade-off accepted: exported Markdown references app-relative attachment URLs, so the git export is not self-contained for images — revisit if export portability becomes a requirement |
| 2026-07-15 | **Document types removed entirely** — the `DocumentType` enum and `documents.type` column are dropped (migration `drop_document_type`), along with the Type field on document creation, the Type search facet, the Type column in the documents/search tables, and the `type:` line in exported Markdown front matter. Auto-assigned doc codes now use a single `DOC-` prefix instead of deriving one per type (`POL-`/`SOP-`/`WI-`/`STD-`); codes already assigned keep their historical prefixes and the manual doc-code field still accepts any 2–6 letter prefix. The 8 starter templates drop their hardcoded `POL-`/`SOP-` codes and are seeded with allocated `DOC-` codes, with idempotency keyed on **title** rather than doc code — sharing one sequence with user documents means a hardcoded code would collide with whatever the org already created and silently skip that template. **Sidebar gains a primary "New document" button above Home** (editors/admins only) | User decision: classification added friction at creation without earning its keep — the folder already carries the grouping a single-org library needs, and the type was write-once metadata nobody filtered on. Accepted trade-off: existing type values are unrecoverable, and doc codes no longer encode the kind of document at a glance |

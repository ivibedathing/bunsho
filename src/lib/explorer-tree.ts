/**
 * The Explorer tree's pure view-model: shapes and the flat-rows→hierarchy
 * assembly, with **no data-access imports**. Split out from `explorer.ts` so the
 * client `ExplorerTree` component can import `countPages` and these types without
 * pulling Prisma (and `pg`, which has no browser build) into the client bundle.
 *
